...
type OrderDetail struct {
	...
	Logs             []OrderLogJoinModel `json:"logs"`
...
type OrderLogModel struct {
	ID          int64     `gorm:"primary_key;auto_increment;unique" json:"id" db:"orderitem_id"`
	OrderID     int64     `gorm:"size:11" json:"order_id"`
	OrderstatID int64     `gorm:"size:11" json:"status_id"`
	Notes       string    `gorm:"size:255" json:"notes"`
	CreatedBy   string    `gorm:"size:20" json:"created_by"`
	CreatedAt   time.Time `gorm:"default:CURRENT_TIMESTAMP" json:"created_at"`
}
type OrderLogJoinModel struct {
	ID         int64  `gorm:"primary_key;auto_increment;unique" json:"id" db:"orderitem_id"`
	OrderID    int64  `gorm:"size:11" json:"order_id"`
	StatusID   int64  `gorm:"size:11" json:"status_id"`
	Status     string `gorm:"size:11" json:"status"`
	StatusDesc string `gorm:"size:11" json:"status_description"`
	Notes      string `gorm:"size:255" json:"notes"`
	CreatedBy  string `gorm:"size:20" json:"created_by"`
	CreatedAt  string `gorm:"default:CURRENT_TIMESTAMP" json:"created_at"`
}
...
func OrderItemSave(tx *gorm.DB, orderItems []OrderItems) error {
	...
	stmt := fmt.Sprintf("INSERT INTO order_items (order_id, product_id, price, qty, weight, sub_price, orderstatitem_id, created_by) VALUES %s",
...
func OrderLogSave(tx *gorm.DB, req OrderLogModel) (err error) {
	sql := `INSERT INTO order_logs (order_id, orderstat_id, notes, created_by) VALUES (?, ?, ?, ?)`

	err = tx.Debug().Exec(sql, req.OrderID, req.OrderstatID, req.Notes, req.CreatedBy).Error
	return err
}

func GetOrderLogByOrderID(db *gorm.DB, OrderID int64) (resp []OrderLogJoinModel, err error) {
	err = db.Debug().Raw(`SELECT 
	ol.id,
	ol.order_id,
	ol.orderstat_id as status_id,
	os.status_name as status,
	os.description as status_desc,
	DATE_FORMAT(ol.created_at, '%Y-%m-%d %H:%i') as created_at,
	created_by
	FROM order_logs ol 
	LEFT JOIN order_status os ON ol.orderstat_id = os.orderstat_id 
	WHERE ol.order_id = ? `, OrderID).Scan(&resp).Error
