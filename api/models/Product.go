...
func (val *Product) GetProduct(db *gorm.DB, start int64, limit int64, type_product string) (res []Product, err error) {
	...
	sql := db.Raw(query, params).Debug().Scan(&res)
	if sql.Error != nil {
...
func (val *Product) GetProducts(db *gorm.DB, par ReqProduct) (interface{}, error) {
	...
	err = db.Raw(`SELECT product_id as id, sku_code, name, img, price, price_disc, 100-ROUND((price_disc / price)* 100, 0) as discount, price_grosir, stock, status_product, description, weight, is_po, po_time, is_warranty, is_new, CONCAT('/product/', slug, '-', product_id) as link, 1 as favorite, 2 as rating, 123 as review, cond AS product_cond FROM products a WHERE is_publish = ? AND CONCAT(name, sku_code, slug, status_product) like '%`+par.Filter.Q+`%' `+_sqlWhere+_sqlOrder, 1).Offset(par.Start).Limit(par.Limit).Scan(&result).Error
	return &result, err
}

func (p *Product) ProductByID(db *gorm.DB, productID int32) (result Product, err error) {
	err = db.Model(Product{}).Where("product_id = ?", productID).Take(&p).Error
	if err != nil {
		return result, err
	}

	db.Raw(`SELECT a.product_id as id, a.sku_code, a.name, a.img, a.price, (CASE WHEN a.cond = 'NEW' THEN 'Baru' ELSE 'Bekas' END) AS product_cond, a.price_disc, a.price_grosir, a.stock, status_product, a.description, a.weight, a.is_po, a.po_time, a.is_warranty, a.is_new, CONCAT('/product/',slug, '-', a.product_id) as link, 1 as favorite, 2 as rating, 123 as review, a.cond AS product_cond, a.sold, b.name as brand 
	FROM products a 
	LEFT JOIN brands b ON a.brand_id = b.id WHERE a.product_id = ? AND a.is_publish = ?`, productID, 1).Take(&result)
	return result, err
}
