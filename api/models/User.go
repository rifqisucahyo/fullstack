...
type User struct {
	...
	Birthday  string    `gorm:"size:100" json:"birthday"`
...
type Address struct {
	...
	Label           string `gorm:"size:20" json:"label"`
...
func (val *User) FindAllUsers(db *gorm.DB) (*[]User, error) {
	var err error
	user := []User{}
	db.Raw("SELECT a.user_id as id, a.username, a.name, a.email, a.phone, a.birthday, a.gender, date_format(a.created_at, '%d %M %Y %H:%i') as created_at, date_format(a.updated_at, '%d %M %Y %H:%i') as updated_at FROM users a").Scan(&user)
	return &user, err
}
