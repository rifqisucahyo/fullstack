...
func (s *Server) saveCheckout(userID uint32, reqCheckout ReqCheckout, resCheckout models.ResCheckout) (*models.Order, error) {
	...
	err = models.OrderLogSave(tx, models.OrderLogModel{
	...
	_, err = s.sendWhatsapp(order.Phone, `Yeey.., Selamat yah orderan kamu berhasil, segera lakukan pembayaran agar barang bisa langsung dikirim, `+os.Getenv("HOST_WEB")+`/order/complete?id=`+fmt.Sprint(order.OrderID))
	if err != nil {
