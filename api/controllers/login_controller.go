...
func (server *Server) SignInPhone(phone string, code int64) (map[string]interface{}, error) {
	...
	user := models.User{}
	err = server.DB.Debug().Model(models.User{}).Select("user_id as id, password, username, name, phone, email, DATE_FORMAT(birthday, '%Y-%m-%d') as birthday, gender, created_at, updated_at").Where("phone = ?", phone).Take(&user).Error
	if err != nil {
	...
	// user.Birthday = user.Birthday.Format("2006-01-02")
